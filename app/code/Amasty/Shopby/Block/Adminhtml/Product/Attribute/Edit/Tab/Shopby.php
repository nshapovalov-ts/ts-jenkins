...
class Shopby extends \Magento\Backend\Block\Widget\Form\Generic
{
    ...
    protected function _prepareForm()
    {
        ...
        $isShowSeperator = $fieldsetDisplayProperties->addField(
            'is_show_after_seperator',
            'select',
            [
                'name'   => 'is_show_after_seperator',
                'label'  => __('Is Show Seperator'),
                'title'  => __('Is Show Seperator'),
                'values' => $this->yesNo->toOptionArray(),
                'note'   => __("This option will only application for sidemenu checkbox filter's")
            ]
        );

        $dependence->addFieldMap(
            $isShowSeperator->getHtmlId(),
            $isShowSeperator->getName()
        );
