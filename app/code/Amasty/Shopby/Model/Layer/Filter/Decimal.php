...
class Decimal extends \Magento\CatalogSearch\Model\Layer\Filter\Decimal implements FromToFilterInterface
{
    ...
    protected function _getItemsData()
    {
        ...
        foreach ($facets as $key => $aggregation) {
            ...
            if ($attribute->getAttributeCode() == "min_order_amount" && !$from) {
                $label = __('No Minimum Order Amount');
                $to = 1;
            }
    ...
    {
        $minOrderAmountFilterCode = \Amasty\Shopby\Helper\FilterSetting::ATTR_PREFIX . 'min_order_amount';
        if ($filterSetting->getFilterCode() == $minOrderAmountFilterCode && $fromPrice == 0) {
            return __('No Minimum Order Amount');
        }
        $formattedFromPrice = $this->priceCurrency->format($this->formatLabelForStateAndRange($fromPrice, $filterSetting));
        if ($toPrice === '') {
            ...
            $result =  __(
                '%1 - %2',
                $formattedFromPrice,
                $this->priceCurrency->format($this->formatLabelForStateAndRange($toPrice, $filterSetting))
            );
        }
