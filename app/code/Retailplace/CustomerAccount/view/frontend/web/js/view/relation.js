...
define([
        ...
        return {
            ...
            runDependencies: function (element, elementId) {
                ...
                $.each(elementDependencies, function(key, relation) {
                    if (this.getElement(relation.depend_id).length) {
                        ...
                        } else if (this.indexedElements.indexOf(relation.depend_id) < 0 && this.isCanHide(relation)) {
            ...
            /**
             * Check if we can hide dependent block.
             *
             * @param {Object} relation
             * @returns {boolean}
             */
            isCanHide: function(relation) {
                var changeVisibility = true;

                $.each(this.config, function(key, item) {
                    if (item.depend_attribute_code == relation.depend_id) {
                        let selector = item.parent_attribute_element_uid + '-' + item.parent_option_id;
            ...
            checkCheckbox: function (elementSet, relation) {
                ...
                $.each(elementSet, function(key, element) {
                    if (!result) {
                        element = $(element);
                        result = element.val() == relation.value && element.is(':checked') === true;
                    }
                });
                return result;
            },
            ...
            checkSelect: function (element, relation) {
                ...
                return element.val() != void(0) && element.val().indexOf(relation.value) != -1
            },
