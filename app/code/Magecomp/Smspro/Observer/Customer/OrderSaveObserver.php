    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if($order)
        {
            ...
            if ($this->helperorder->isOrderNotificationForUser($order->getStoreId()))
            {
                ...
                $dltid = $this->helperorder->getOrderNotificationUserDltid($order->getStoreId());
            ...
            if($this->helperorder->isOrderNotificationForAdmin($order->getStoreId()) && $this->helperorder->getAdminNumber($order->getStoreId()))
            {
                ...
                $dltid = $this->helperorder->getOrderNotificationForAdminDltid($order->getStoreId());

                $finalmessage = $this->emailfilter->filter($message);
                $this->helperapi->callApiUrl($this->helperorder->getAdminNumber($order->getStoreId()),$finalmessage,$dltid);
            }
        }
        return $this;
    }
