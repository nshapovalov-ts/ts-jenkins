...
class ListingTabsWeeklydeals extends \Magento\Catalog\Block\Product\AbstractProduct
{
	...
	public function _getProductsBasic($catids = null,  $tab = false)
	{
        ...
        if (!empty($catids) && $catids){
			...
			$allsku = null;
            $miraklshopTable = $resource->getTableName('mirakl_shop');
            $miracalshopdata = $dbconnection->fetchAll("SELECT * FROM ".$miraklshopTable."");

			foreach ($miracalshopdata as $shopvalue) {
                $strshop[$shopvalue['id']] = $shopvalue['id'];
            }
            $random_keys=array_rand($strshop,count($strshop));
            $clerenchome = 1;
            foreach ($random_keys as $value) {
            	if($clerenchome >= 12){
            		break;
            	}
            	$product_skus = $dbconnection->fetchAll("SELECT * FROM ".$miraklOfferTable." WHERE `shop_id` = '".$value."' ORDER BY price ");
			            foreach ($product_skus as $value) {
						$str = $value['additional_info'];
						$checkclerenc = explode(",",$str);
						$count = $dbconnection->fetchOne("SELECT COUNT(*) FROM  ".$miraklOfferTable." WHERE `product_sku` = '".$value['product_sku']."'");
							if($count=='1'){
							if($value['price'] < $value['origin_price']){
			                    if($checkclerenc['0'] != '{"clearance":"true"'){
			                        if($checkclerenc['0'] != '{"clearance":"true"}'){
			                            $allsku[] = $value['product_sku'];
			                            $clerenchome++;
			                            break;
			                        }
			                   	}
        ...
        $collection->addAttributeToFilter('sku', array('in' => $allsku));
