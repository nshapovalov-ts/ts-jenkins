...
namespace Kipanga\Miraklrefund\Rewrite;
...
class CreditmemoFactory extends \Magento\Sales\Model\Order\CreditmemoFactory
{
     ...
     * @return Creditmemo
     */
    public function createByOrder(\Magento\Sales\Model\Order $order, array $data = [], array $orderItemIds = [])
    {
        $totalQty = 0;
        $creditmemo = $this->convertor->toCreditmemo($order);
        $qtyList = isset($data['qtys']) ? $data['qtys'] : [];
        $adjustmentExact = $data['adjustment_exact'] ?? 0;
        foreach ($order->getAllItems() as $orderItem) {
            ...
            if ($orderItemIds) {
                if ((in_array($orderItem->getId(), $orderItemIds) || in_array($orderItem->getParentItemId(), $orderItemIds))) {
                    //$orderItem->setData('row_invoiced',$adjustmentExact);
     ...
     * @return Creditmemo
     */
    public function createByInvoice(\Magento\Sales\Model\Order\Invoice $invoice, array $data = [], array $orderItemIds = [])
    {
