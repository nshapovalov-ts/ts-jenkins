...
class Login extends AbstractSocial
{
    ...
    public function execute()
    {
        ...
        $customer = $this->apiObject->getCustomerBySocial($userProfile->identifier, $type, $email);
        ...
        if (!$customer->getId()) {
            ...
            if ($customer->getData('is_not_approved')) {
                $message = __('The account has not been approved yet please contact <a href="mailto:support@tradesquare.com.au">support@tradesquare.com.au</a> to learn more.');
                $this->messageManager->addMessage(
                    $this->messageManager->createMessage(MessageInterface::TYPE_ERROR)
                        ->setText($message)
                        ->setData(['view_type' => 'error_registering'])
                );
            } else {
                $url = $this->_url->getDirectUrl('sign-up-page');
                $message = __('Could not find your account, please <a href="%1"> sign up</a> or try again.', $url);
                $this->messageManager->addMessage(
                    $this->messageManager->createMessage(MessageInterface::TYPE_ERROR)
                        ->setText($message)
                        ->setData(['view_type' => 'error_registering'])
