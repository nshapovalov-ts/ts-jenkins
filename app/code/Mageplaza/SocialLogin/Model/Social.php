...
class Social extends AbstractModel
{
     ...
     * @return Customer
     */
    public function getCustomerBySocial($identify, $type, $email = "")
    {
        ...
        if ($customer->getId()) {
            $this->setAuthorCustomer($identify, $customer->getId(), $type);
        }
        if ($customerId = $customer->getId()) {
            $getIsApproved = $this->helperData->getIsApproved($customerId);
            if ($getIsApproved == "notapproved" || !$getIsApproved) {
                $customer = $this->customerFactory->create();
                $customer->setData('is_not_approved', true);
            }
        }
        return $customer;
    }
